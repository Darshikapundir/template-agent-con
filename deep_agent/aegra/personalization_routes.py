...
async def delete_memory(request: Request, memory_id: str) -> None:
    ...
    for item in items:
        ...
        for fact in facts:
            ...
            if not cleaned:
                continue
            fact_id = hashlib.sha256(f"{key}:{cleaned}".encode()).hexdigest()[:12]
...
async def create_rule(request: Request, body: RuleCreate) -> RuleOut:
    ...
    user_id = _get_user_id(request)
    repo = _get_repo()
    count = await repo.count_rules(user_id)
    if count >= MAX_RULES_PER_USER:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail=f"Maximum {MAX_RULES_PER_USER} rules allowed per user",
        )
    try:
